...
/// 物品放入物品槽Demo
/// </summary>
public class ItemDragDemo : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    private RectTransform m_RectTransform;      //获取该组件，用于拖拽时移动自身位置
    private Text m_Text;                        //用于修改自身是否接收射线
    ...
    {
        m_Text.raycastTarget = false;   //取消接收事件
    ...
    {
        m_Text.raycastTarget = true;    //恢复接收事件
